...
namespace TheOrganizer
{
    ...
    internal class Hotkey : IDisposable
    {
        ...
        public Action<HotkeyEventArgs> HotkeyPressed;

        public readonly KeyModifier ModifierKeys;
        public readonly VirtualKey Key;
        public readonly bool NoRepeat;
        public readonly ushort Id;

        public Hotkey(KeyModifier modifierKeys, VirtualKey key, Action<HotkeyEventArgs> action, bool noRepeat)
        {
            HotkeyPressed = action;
            ModifierKeys = modifierKeys;
            Key = key;
            NoRepeat = noRepeat;
            Id = NativeMethods.GlobalAddAtom(Guid.NewGuid().ToString());
        }
